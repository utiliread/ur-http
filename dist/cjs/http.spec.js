...
describe("static http", function () {
    it("can get", function () {
        ...
        chai_1.expect(builder.options.fetch).to.equal(http_1.Http.defaults.fetch);
    });
});
describe("instance http", function () {
    it("can get with different timeout", function () {
        ...
        chai_1.expect(builder.options.fetch).to.equal(http_1.Http.defaults.fetch);
        chai_1.expect(builder.options.timeout).to.equal(undefined);
    });
    it("can get with different timeout", function () {
        var http = new http_1.Http({ timeout: 123 });
        var builder = http.get("/hello");
        chai_1.expect(builder.options.fetch).to.equal(http_1.Http.defaults.fetch);
        chai_1.expect(builder.options.timeout).to.equal(123);
    });
    it("can get with different fetch", function () {
        var http = new http_1.Http({ fetch: fakeFetch });
        var builder = http.get("/hello");
        chai_1.expect(builder.options.fetch).to.equal(fakeFetch);
        chai_1.expect(builder.options.timeout).to.equal(undefined);
    });
});
